  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl p-0 gap-0 overflow-hidden" dir="rtl">
        ...
        {/* زر الإغلاق - يمين (نفس لون Sheet) */}
        <button
          onClick={() => onOpenChange(false)}
          className="absolute top-4 right-4 z-50 w-10 h-10 rounded-full bg-rose-500 hover:bg-rose-600 active:bg-rose-700 flex items-center justify-center text-white shadow-md transition-colors"
          aria-label="إغلاق"
          title="إغلاق"
          style={{ right: "1rem", top: "calc(1rem + env(safe-area-inset-top))" }}
        >
